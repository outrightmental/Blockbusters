...
func do_break() -> void:
	# Quarter A
	var fragA: Node = (quart_scene_1a if half_num == 1 else quart_scene_2a).instantiate()
	fragA.add_collision_exception_with(self)
	fragA.position = position
	fragA.linear_velocity = linear_velocity
	# Quarter B
	var fragB: Node = (quart_scene_1b if half_num == 1 else quart_scene_2b).instantiate()
	fragB.add_collision_exception_with(self)
	fragB.position = position
	fragB.linear_velocity = linear_velocity
	# Remove the block from the scene
	self.get_parent().call_deferred("add_child", fragA)
	self.get_parent().call_deferred("add_child", fragB)
