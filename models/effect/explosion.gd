...
func _ready() -> void:
	...
	var candidates: Array[Node] = get_tree().get_nodes_in_group(Global.GROUP_AFFECTED_BY_EXPLOSION)
	for target in candidates:
		var diff: Vector2 = (target.position - position)
		# Check if the collidable is within the explosion radius
# TODO		if diff.length() <= EXPLOSION_RADIUS:
# TODO			# Apply a force to the collidable
# TODO			target.apply_central_force(diff.normalized() * EXPLOSION_FORCE * (1 - diff.length() / EXPLOSION_RADIUS))
