...
func _test_fully_cooked(_p1: Ship, p2: Ship) -> Signal:
	_begin("Player 1 fires the laser directly at Player 2 until Player 2 is disabled (no heat)")
...
#
func _test_cannot_overcook(p1: Ship, p2: Ship) -> Signal:
	_begin("Player 1 fires the entire laser charge directly at Player 2 -- Player 2 disabled (cannot heat while disabled)")
	InputManager.action_pressed.emit(1, InputManager.INPUT_ACTION_A)
	while p1.is_laser_active():
...
func _test_half_baked(p1: Ship, _p2: Ship) -> Signal:
	_begin("Player 2 fires the laser directly at Player 1 until Player 1 is heated about 50%, not disabled")
...
func _test_grazed(_p1: Ship, p2: Ship) -> Signal:
	_begin("Player 1 fires the laser directly at Player 2 until Player 2 is heated about 20%, not disabled")
	InputManager.action_pressed.emit(1, InputManager.INPUT_ACTION_A)
